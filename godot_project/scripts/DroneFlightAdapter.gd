...
func _ready():
	...
	print("Node type: AeroBody3D")
	print("Using aerodynamic physics")
	...
	# Configure AeroBody3D properties
	print("Configuring AeroBody3D-specific properties")
	control_command = Vector3.ZERO
	throttle_command = 0.0
	
	# Enable debug visualization safely
	call_deferred("_enable_debug_safely")
	
	# Setup aerodynamic components
	_setup_aero_physics()
	
	# Initialize with no target position to trigger fallback behavior
	target_position = Vector3.ZERO

func _enable_debug_safely():
	"""Safely enable debug visualization after initialization"""
	# Skip debug visualization to avoid null reference errors during initialization
	# The AeroBody3D plugin's debug vectors aren't properly initialized yet
	print("Skipping debug visualization to avoid initialization errors")
	# Uncomment these lines once the plugin is fully stable:
	# await get_tree().process_frame
	# show_debug = true
	# show_lift_vectors = true
	# show_drag_vectors = true

func _setup_aero_physics():
	"""Setup for AeroBody3D physics"""
	# Create aerodynamic components for the drone
	_create_aerodynamic_rotors()
	_create_aerodynamic_surfaces()
	print("Aerodynamic components created - using enhanced physics")

func _create_aerodynamic_rotors():
	"""Create aerodynamic rotor components for realistic thrust"""
	# Only create if aerodynamic classes are available
	if not ClassDB.class_exists("AeroPropeller3D"):
		print("AeroPropeller3D class not found")
		return
	
	var AeroPropeller3D = load("res://addons/godot_aerodynamic_physics/core/aero_influencer_3d/aero_rotator_3d/aero_propeller_3d.gd")
	if not AeroPropeller3D:
		print("Could not load AeroPropeller3D script")
		return
	
	var rotor_positions = _get_rotor_positions()
	for i in range(rotor_count):
		var propeller = AeroPropeller3D.new()
		propeller.name = "AeroPropeller_" + str(i)
		propeller.position = rotor_positions[i]
		
		# Configure propeller properties for quadcopter rotor
		propeller.blade_count = 2
		propeller.radius = 0.15  # 15cm rotor radius
		propeller.blade_chord = 0.02  # 2cm blade chord
		propeller.collective_pitch = 5.0  # 5 degree blade angle
		propeller.max_rpm = max_rotor_speed
		
		add_child(propeller)
		print("Created aerodynamic rotor ", i, " at ", rotor_positions[i])

func _create_aerodynamic_surfaces():
	"""Create aerodynamic surface components for stability"""
	# Only create if aerodynamic classes are available
	if not ClassDB.class_exists("AeroSurface3D"):
		print("AeroSurface3D class not found")
		return
	
	var AeroSurface3D = load("res://addons/godot_aerodynamic_physics/core/aero_influencer_3d/aero_surface_3d/aero_surface_3d.gd")
	var AeroSurfaceConfig = load("res://addons/godot_aerodynamic_physics/core/aero_influencer_3d/aero_surface_3d/aero_surface_config.gd")
	
	if not AeroSurface3D or not AeroSurfaceConfig:
		print("Could not load AeroSurface3D or AeroSurfaceConfig scripts")
		return
	
	# Create small stability surfaces (like fins)
	...
	for i in range(4):
		...
		surface.name = "AeroSurface_" + str(i)
		surface.position = surface_positions[i]
		
		# Create small wing config for stability
		var config = AeroSurfaceConfig.new()
		config.chord = 0.05  # 5cm chord
		config.span = 0.05   # 5cm span
		config.aspect_ratio = 1.0
		config.lift_curve_slope = 4.0
		config.zero_lift_drag = 0.01
		config.induced_drag_factor = 0.9
		
		surface.aero_surface_config = config
		add_child(surface)
		print("Created aerodynamic surface ", i, " at ", surface_positions[i])
...
func _physics_process(delta: float):
	...
	# Update flight control using AeroBody3D
	_update_aero_flight_control(delta)
...
func _update_aero_flight_control(delta: float):
	"""Flight control using AeroBody3D features"""
	# Use native AeroBody3D control properties
	# AeroBody3D uses: X = Pitch, Y = Yaw, Z = Roll
	control_command = Vector3(pitch_input, yaw_input, roll_input)
	...
	if fmod(Time.get_time_dict_from_system().get("second", 0), 3) == 0:
		print("AeroBody3D control - Pitch:", pitch_input, " Roll:", roll_input, " Yaw:", yaw_input, " Throttle:", throttle_input)
